...
_RUNTIME_CCE_IMPORT_ERROR = None   # Cache import failure so we don't retry
_RUNTIME_CCE_MAKE_LOSS = None      # Cached reference to make_chunked_cross_entropy_loss
_RUNTIME_CCE_CACHE = {}           


def _load_runtime_cce():
    """Lazily import mlx_cce_runtime.make_chunked_cross_entropy_loss.

    Returns the factory function on success, None if the package is missing.
    Caches both success and failure so subsequent calls are free.
    """
    global _RUNTIME_CCE_IMPORT_ERROR, _RUNTIME_CCE_MAKE_LOSS
...
def _get_runtime_cce(
    *,
    ...
    """Get a cached runtime-cce loss callable for the given configuration.

    Returns None if mlx_cce_runtime is not available.
    """
    ...
    cached = _RUNTIME_CCE_CACHE.get(key)
    ...
    _RUNTIME_CCE_CACHE[key] = loss_fn
    return loss_fn
...
def has_cce_kernel():
    """Check if runtime CCE (mlx_cce_runtime) is available."""
    return _load_runtime_cce() is not None
...
def make_cce_loss_fn(model):
    """Create a CCE loss function using mlx_cce_runtime.
    ...
    if not has_cce_kernel():
        raise RuntimeError(
            "No CCE backend available. Install runtime-cce:\n"
